...
class CacheFS {
     ...
     * @return {string}
     */
    GetCachePath(guid, hash, extension, create, callback) {
        var dir = this.cacheDir + "/" + guid.substring(0, 2);
        var path = dir + "/" + guid + "-" + hash + "." + extension;
        ...
        if (create) {
            ...
            process.nextTick(callback.bind(null, null, path));
        }

        if(!callback) return path;
    }
    
    GetCacheFileStream(cachePath) {
        var file = fs.createReadStream(cachePath);
        ...
        file.on('close', function () {
            try {
                ...
                helpers.log(consts.LOG_DBG, "Updating mtime of " + cachePath + " to: " + dateNow);
                fs.utimesSync(cachePath, dateNow, dateNow);
            }
            catch (err) {
                helpers.log(consts.LOG_ERR, "Failed to update mtime of " + cachePath + ": " + err);
            }
        });

        file.on('open', function (fd) {
            fs.fstat(fd, function (err, stats) {
                file.emit('cache_open', err, stats.size);
            });
        });
