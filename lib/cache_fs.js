...
class CacheFS {
    ...
    _InitCache() {
        ...
        globals.log(consts.LOG_DBG, "Cache Server directory " + this.cacheDir);
        globals.log(consts.LOG_DBG, "Cache Server size " + this.totalDataSize);
        globals.log(consts.LOG_DBG, "Cache Server max cache size " + this.maxCacheSize);
    ...
    _FreeSpace(freeSize) {
        if (this._freeingSpaceLock != 0) {
            globals.log(consts.LOG_DBG, "Skip free cache space because it is already in progress: " + this._freeingSpaceLock);
            return;
        }

        this._LockFreeSpace();

        globals.log(consts.LOG_TEST, "Begin freeing cache space. Current size: " + this._totalDataSize);

        var self = this;
        this._WalkDirectory(this._cacheDir, function (err, files) {
            if (err)
                throw err;
            ...
            while (self._totalDataSize > freeSize) {
    ...
    _UnlockFreeSpace() {
        this._freeingSpaceLock--;
        if (this._freeingSpaceLock == 0) {
            globals.log(consts.LOG_TEST, "Completed freeing cache space. Current size: " + this._totalDataSize);
        }
    }
    ...
    _ValidateFile(dir, file, fix) {
        ...
        if (matches == null) {
            let path = dir ? this._cacheDir + "/" + dir + "/" + file : this._cacheDir + "/" + file;
        ...
        if (matches[1].toLowerCase() != dir.toLowerCase()) {
            let path = this._cacheDir + "/" + dir + "/" + file;
        ...
        if (matches[4].toLowerCase() == "info" || matches[4].toLowerCase() == "resource") {
            let checkedPath = this._cacheDir + "/" + dir + "/" + matches[1] + matches[2] + "-" + matches[3] + ".bin";
            try {
                fs.accessSync(checkedPath);
            }
            catch (e) {
                let path = this._cacheDir + "/" + dir + "/" + file;
        ...
        if (matches[4].toLowerCase() == "bin" || matches[4].toLowerCase() == "resource") {
            let checkedPath = this._cacheDir + "/" + dir + "/" + matches[1] + matches[2] + "-" + matches[3] + ".info";
            try {
                fs.accessSync(checkedPath);
            }
            catch (e) {
        ...
        if (matches[4].toLowerCase() == "info") {
            try {
                var contents = fs.readFileSync(this._cacheDir + "/" + dir + "/" + file, "ascii");
                if (contents.indexOf("assetImporterClassID: 1020") > 0) {
                    var checkedPath = this._cacheDir + "/" + dir + "/" + matches[1] + matches[2] + "-" + matches[3] + ".resource";
                    try {
                        fs.accessSync(checkedPath);
                    }
                    catch (e) {
                        var path = this._cacheDir + "/" + dir + "/" + file;
                        var msg = "Missing audio file " + checkedPath + " for " + path + ".";
                        CacheFS.FixFileIfRequired(path, msg, fix);
                        path = this._cacheDir + "/" + dir + "/" + matches[1] + matches[2] + "-" + matches[3] + ".bin";
    ...
    _RenameFileSync(from, to, size, oldSize) {
        try {
            ...
            this.AddFileToCache(size - oldSize);
        }
        catch (err) {
    ...
    AddFileToCache(bytes) {
        if (bytes != 0) {
            this._totalDataSize += bytes;
            globals.log(consts.LOG_DBG, "Total Cache Size " + this.totalDataSize);

            if (this._totalDataSize > this.maxCacheSize)
                this._FreeSpace(this.GetFreeCacheSize());
        }
    }
    ...
    VerifyCache(fix) {
        ...
        return this._VerifyCacheDirectory(null, this.cacheDir, fix);
    };
    ...
    FreeSpaceAfterWriteFailure() {
        return this._FreeSpace(this._totalDataSize * freeCacheSizeRatioWriteFailure);
    };
}
