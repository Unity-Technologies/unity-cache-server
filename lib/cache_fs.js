...
var maxCacheSize = 1024 * 1024 * 1024 * 50; // 50Gb
...
function ValidateFile(dir, file, fix) {
    ...
    if (matches == null) {
        let path = dir ? cacheDir+"/"+dir+"/"+file : cacheDir+"/"+file;
    ...
    fs.readdirSync(dir).forEach(function (file) {
        ...
        if (stats.isDirectory()) {
            if (!ShouldIgnoreFile(file)) {
                ...
                if (parent == null)
                    errCount += VerifyCacheDirectory(file, path, fix)
            }
        }
        else if (stats.isFile()) {
            errCount += ValidateFile(parent, file, fix);
        }
    });
...
function RenameFileSync(from, to, size, oldSize) {
    try {
...
exports.VerifyCache = function(fix) {
    ...
    return VerifyCacheDirectory(null, cacheDir, fix);
};
...
exports.ReplaceFileSync = function(from, to, size) {
    var stats = {};
    try {
        ...
        try {
            ...
            RenameFileSync(from, to, size, oldSize);
        }
        catch(err) {
