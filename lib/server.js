...
function handleData(socket, data, cache) {
    ...
    while (true) {
                ...
                socket.activePutFile.end(function () {
                    ...
                    if (socket.isActive) {
                        ...
                        if (socket.inTransaction)
                            handleData(socket, Buffer.from([]), cache);
                    }
                });
        ...
        if (data[idx] == CMD_GET) {
            ...
            if (reqType == TYPE_ASSET) {
                ...
                socket.getFileQueue.unshift({
                    buffer: resbuf,
                    type: TYPE_ASSET,
                    cacheStream: cache.GetCachePath(guid, hash, 'bin', false)
                });
            }
                ...
                socket.getFileQueue.unshift({
                    buffer: resbuf,
                    type: TYPE_INFO,
                    cacheStream: cache.GetCachePath(guid, hash, 'info', false)
                });
            }
                ...
                socket.getFileQueue.unshift({
                    buffer: resbuf,
                    type: TYPE_RESOURCE,
                    cacheStream: cache.GetCachePath(guid, hash, 'resource', false)
                });
            }
            else {
        ...
        // handle a transaction
            ...
            if (data[idx] == TRX_START) {
                ...
                for (let i = 0; i < socket.targets.length; i++) {
                    ...
                    cache.ReplaceFileSync(socket.targets[i].from, socket.targets[i].to, socket.targets[i].size);
                }
        ...
        // Put a file from the client to the cache server
            ...
            if (reqType == TYPE_ASSET) {
                ...
                socket.activePutTarget = cache.GetCachePath(socket.currentGuid, socket.currentHash, 'bin', true);
            }
            else if (reqType == TYPE_INFO) {
                globals.log(consts.LOG_TEST, "Put Asset Info " + socket.currentGuid + "-" + socket.currentHash + " (size " + size + ")");
                socket.activePutTarget = cache.GetCachePath(socket.currentGuid, socket.currentHash, 'info', true);
            }
            else if (reqType == TYPE_RESOURCE) {
                globals.log(consts.LOG_TEST, "Put Asset Resource " + socket.currentGuid + "-" + socket.currentHash + " (size " + size + ")");
                socket.activePutTarget = cache.GetCachePath(socket.currentGuid, socket.currentHash, 'resource', true);
            }
            else {
                globals.log(consts.LOG_ERR, "Invalid data receive");
                socket.destroy();
            ...
            socket.tempPath = cache.cacheDir + "/Temp" + uuid();
            ...
            socket.activePutFile.on('error', function (err) {
                ...
                cache.FreeSpaceAfterWriteFailure();
                socket.destroy();
                return false;
            });
            socket.bytesToBeWritten = size;
        ...
        // handle check integrity
            ...
            if (data[idx] == CMD_CHECK && (data[idx + 1] == OPT_VERIFY || data[idx + 1] == OPT_FIX)) {
                ...
                let verificationNumErrors = cache.VerifyCache(fixIt);
                if (fixIt)
 ...
 * @param logFn log function (optional)
 * @param errCallback error callback (optional)
 */
exports.Start = function (cache, port, errCallback) {
    ...
    var server = net.createServer(function (socket) {
        ...
        socket.on('close', function (had_errors) {
            ...
            var checkFunc = function () {
                ...
                if (handleData(socket, data, cache)) {
    ...
    server.on('error', function (e) {
        if (e.code == 'EADDRINUSE') {
            globals.log(consts.LOG_ERR, 'Port ' + port + ' is already in use...');
            if (errCallback) {
                errCallback(e);
            }
        }
