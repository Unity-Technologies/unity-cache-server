...
const cachefs = require('./cache_fs');
    ...
    while (true) {
        ...
        if (data[idx] == CMD_GET) {
            ...
            if (reqType == TYPE_ASSET) {
                ...
                socket.getFileQueue.unshift({
                    buffer: resbuf,
                    type: TYPE_ASSET,
                    cacheStream: cachefs.GetCachePath(guid, hash, 'bin', false)
                });
            }
                ...
                socket.getFileQueue.unshift({
                    buffer: resbuf,
                    type: TYPE_INFO,
                    cacheStream: cachefs.GetCachePath(guid, hash, 'info', false)
                });
            }
                ...
                socket.getFileQueue.unshift({
                    buffer: resbuf,
                    type: TYPE_RESOURCE,
                    cacheStream: cachefs.GetCachePath(guid, hash, 'resource', false)
                });
            }
            else {
        ...
        // handle a transaction
            ...
            if (data[idx] == TRX_START) {
                ...
                for (let i = 0; i < socket.targets.length; i++) {
                    globals.log(consts.LOG_DBG, "Rename " + socket.targets[i].from + " to " + socket.targets[i].to);
                    cachefs.ReplaceFileSync(socket.targets[i].from, socket.targets[i].to, socket.targets[i].size);
                }
        ...
        // Put a file from the client to the cache server
            ...
            if (reqType == TYPE_ASSET) {
                ...
                socket.activePutTarget = cachefs.GetCachePath(socket.currentGuid, socket.currentHash, 'bin', true);
            }
            else if (reqType == TYPE_INFO) {
                globals.log(consts.LOG_TEST, "Put Asset Info " + socket.currentGuid + "-" + socket.currentHash + " (size " + size + ")");
                socket.activePutTarget = cachefs.GetCachePath(socket.currentGuid, socket.currentHash, 'info', true);
            }
            else if (reqType == TYPE_RESOURCE) {
                globals.log(consts.LOG_TEST, "Put Asset Resource " + socket.currentGuid + "-" + socket.currentHash + " (size " + size + ")");
                socket.activePutTarget = cachefs.GetCachePath(socket.currentGuid, socket.currentHash, 'resource', true);
            }
            else {
                globals.log(consts.LOG_ERR, "Invalid data receive");
                socket.destroy();
            ...
            socket.tempPath = cachefs.GetCacheDir() + "/Temp" + uuid();
            ...
            socket.activePutFile.on('error', function (err) {
                ...
                cachefs.FreeSpaceAfterWriteFailure();
                socket.destroy();
                return false;
            });
            socket.bytesToBeWritten = size;
        ...
        // handle check integrity
            ...
            if (data[idx] == CMD_CHECK && (data[idx + 1] == OPT_VERIFY || data[idx + 1] == OPT_FIX)) {
                ...
                let verificationNumErrors = cachefs.VerifyCache(fixIt);
                if (fixIt)
    ...
    file.on('open', function (fd) {
        fs.fstat(fd, function (err, stats) {
            if (err)
                ...
                globals.log(consts.LOG_INFO, "Found: " + next.cacheStream + " size:" + stats.size);
...
exports.GetProtocolVersion = function () {
    return consts.PROTOCOL_VERSION;
};
...
exports.Start = function (a_cacheSize, a_port, a_path, a_logFn, a_errCallback) {
    ...
    cachefs.SetMaxCacheSize(a_cacheSize);
    cachefs.SetCacheDir(a_path);
    cachefs.InitCache();

    let port = parseInt(a_port) || 0;

    server.on('error', function (e) {
        if (e.code == 'EADDRINUSE') {
            globals.log(consts.LOG_ERR, 'Port ' + exports.GetPort() + ' is already in use...');
            if (a_errCallback) {
                a_errCallback(e);
            }
        }
...
exports.Verify = function (a_path, a_logFn, a_fix) {
    ...
    cachefs.SetCacheDir(a_path);
    return cachefs.VerifyCache(a_fix);
};