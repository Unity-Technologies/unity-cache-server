...
const ClusterMessages = require('../cluster_messages');
...
class CacheFS extends CacheBase {
    constructor() {
        super();
        this._cm = new ClusterMessages();
        this._guidRefs = {};

        if(cluster.isMaster) {
            this._cm.listenFor(k_incrementGuidRef, key => {
                this._incrementGuidRef(key);
            });

            this._cm.listenFor(k_releaseGuidRef, key => {
    ...
    _incrementGuidRef(key) {
        if(cluster.isWorker) {
            return this._cm.send(k_incrementGuidRef, key, 1000);
        }
    ...
    _releaseGuidRef(key) {
        if(cluster.isWorker) {
            return this._cm.send(k_releaseGuidRef, key);
        }
