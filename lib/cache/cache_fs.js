...
class CacheFS extends CacheBase {
    ...
    cleanup(dryRun = true) {
        ...
        if(this._options.cleanupOptions.lowMemory) {
            return this.cleanup_low_mem(dryRun, expireDuration, minFileAccessTime, maxCacheSize);
        }

        return this.cleanup_fast(dryRun, expireDuration, minFileAccessTime, maxCacheSize);
    }
    ...
    cleanup_fast(dryRun = true, expireDuration, minFileAccessTime, maxCacheSize) {
        ...
        const progressData = () => {
            return {
                ...
                msg: 'Gathering cache files for expiration',
            };
        };
    ...
    cleanup_low_mem(dryRun = true, expireDuration, minFileAccessTime, maxCacheSize) {
        ...
        return helpers.readDir(self._cachePath, async (item) => {
            ...
            if(item.stats.atime < minFileAccessTime) {
                deleteSize += item.stats.size;
                deletedItemCount++;
                self.emit('cleanup_delete_item', item.path);
                if(!dryRun) {
            ...
            await helpers.readDir(self._cachePath, (item) => {
                if(item.stats.isDirectory()) return next();
                ...
                if (cacheSize - deleteSize > maxCacheSize) {
                    ...
                    if (item.stats.atime > i.stats.atime) {
                        helpers.insertSorted(item, deleteItems, (a, b) => { return a.stats.atime > b.stats.atime });
                        deleteSize += item.stats.size;
