...
class CacheMembuf {
    ...
    static _waitForSerialize() {
        return new Promise((resolve) => {
            (function waitForSave() {
                if(CacheMembuf._searializeInProgess === false) return resolve();
    ...
    static endPutTransaction(transaction, callback) {
        CacheMembuf._waitForSerialize().then(() => {
...
CacheMembuf._searializeInProgess = false;
...
class PersistenceAdapter extends loki.LokiFsAdapter {
    ...
    saveDatabase(dbname, dbstring, callback) {
        CacheMembuf._searializeInProgess = true;
        super.saveDatabase(dbname, dbstring, function() {
            CacheMembuf._serialize(function() {
                CacheMembuf._searializeInProgess = false;
                callback();
            });
        });
