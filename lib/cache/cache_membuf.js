...
class CacheMembuf extends CacheBase {
    ...
    getFileInfo(type, guid, hash, callback) {
        ...
        let p = (entry != null)
            ...
            : Promise.reject(new Error(`File not found for (${type}) ${guid.toString('hex')}-${hash.toString('hex')}`));
    ...
    getFileStream(type, guid, hash, callback) {
        ...
        let p = new Promise((resolve, reject) => {
            ...
            if(entry != null) {
                ...
                reject(new Error(`File not found for (${type}) ${guid.toString('hex')}-${hash.toString('hex')}`));
            }
        });
