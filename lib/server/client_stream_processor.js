...
class ClientStreamProcessor extends Transform {
    ...
    _sendCommands(data) {
        ...
        function fillBufferWithData(size) {
            ...
            size = Math.min(self.readState.headerSize - self.readState.headerBufPos);
        ...
        if(!this.didReadVersion) {
            const verSize = Math.max(consts.VERSION_SIZE, Math.min(consts.PROTOCOL_VERSION_MIN_SIZE, data.length));
            dataPos += verSize;
