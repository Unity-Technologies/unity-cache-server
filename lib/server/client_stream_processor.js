...
class ClientStreamProcessor extends Transform {
    ...
    static get errorCodes() {
        return {
            quitError: new Error("Client quit")
        }
    }
    ...
    _transform(data, encoding, callback) {
        while(data !== null && data.length > 0 && this.errState === null) {
            ...
            if(this.errState !== null) {
                helpers.log(consts.LOG_ERR, this.errState);
                this.push('q'); // quit
            }
        }
    ...
    _sendCommands(data) {
        ...
        function fillBufferWithData() {
            ...
            assert(self.readState.headerBufPos <= self.headerBuf.length);
        ...
        while(!isDone()) {
            ...
            if(!this.readState.didParseCmd) {
                this.readState.didParseCmd = true;
