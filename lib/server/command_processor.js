        ...
        this[kCache].getFileStream(file.type, file.guid, file.hash)
            .then(stream => {
                function readTimeout() {
                    helpers.log(consts.LOG_ERR, "Timeout waiting for readable! Trying to read again");
                    setImmediate(readChunk);
                }

                function readChunk() {
                    self.readFileTimeout && clearTimeout(self.readFileTimeout);
                    ...
                    if(self[kSendFileQueue].length > 0)
                        self.readFileTimeout = setTimeout(readTimeout, 5000);
                }
