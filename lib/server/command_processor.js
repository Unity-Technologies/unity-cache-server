...
class CommandProcessor extends Duplex {
    ...
    constructor(cache) {
        ...
        this._options = config.get("Processor.options");
        this._putWhitelist = this._options.putWhitelist;
        this._whitelistEmpty = false;

        if(!Array.isArray(this._putWhitelist) || !this._putWhitelist.length){
            this._whitelistEmpty = true;
            helpers.log(consts.LOG_INFO, `Allowing all transactions`);
        }
        else{
    ...
    _isWhitelisted(ip) {
        if(this._whitelistEmpty){
            ...
            return this._putWhitelist.indexOf(ip) != -1;
        }
    }
