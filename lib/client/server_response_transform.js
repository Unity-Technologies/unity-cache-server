...
class CacheServerResponseTransform extends Transform {
    ...
    _emitHeader(data, callback) {
        ...
        // Read version
        if(!didWrite('version') && writeHeaderBuffer(consts.VERSION_SIZE)) {
            this.headerData.version = globals.readUInt32(this.headerBuf.slice(0, consts.VERSION_SIZE));
        }

        if(isDone()) { return callback(); }

        // Read command
        if(!didWrite('cmd') && writeHeaderBuffer(consts.CMD_SIZE)) {
        ...
        // Read size
        if(this.doReadSize && !didWrite('size') && writeHeaderBuffer(consts.SIZE_SIZE)) {
            this.headerData.size = globals.readUInt64(this.headerBuf.slice(0, consts.UINT64_SIZE));
        }

        if(isDone()) { return callback(); }

        // Read ID
        if(this.doReadId && !didWrite('guid') && writeHeaderBuffer(consts.ID_SIZE)) {
