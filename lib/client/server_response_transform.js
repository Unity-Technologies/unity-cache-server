    ...
    _emitHeader(data) {
        ...
        while(!isDone()) {
            ...
            if (!this.readState.didReadCmd) {
                ...
                switch (cmd[0]) {
                    ...
                    case '-': // file not found
                        ...
                        break;
                    default:
                        this.errState = new Error("Unrecognized command response, aborting!");
                }
            }
            ...
            if (this.readState.doReadSize) {
                this.readState.headerData.size = helpers.readUInt64(this.headerBuf.slice(pos, pos + consts.UINT64_SIZE));
                pos += consts.UINT64_SIZE;
                this.readState.dataPassThrough = true;
            }

            if(this.readState.doReadId) {
                this.readState.headerData.guid = this.headerBuf.slice(pos, pos + consts.GUID_SIZE);
                pos += consts.GUID_SIZE;
                this.readState.headerData.hash = this.headerBuf.slice(pos, pos + consts.HASH_SIZE);
            }
